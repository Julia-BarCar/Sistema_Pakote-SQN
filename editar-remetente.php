...
<?php 
	$sql = "SELECT * FROM remetente  WHERE id_remetente = ".@$_REQUEST['id_remetente'];
	$res = $conn->query($sql);
	$row = $res->fetch_object();
 ?>
 <form action="?page=salvar-remetente" method="POST">
 	<input type="hidden" name="acao" value="editar">
 	<input type="hidden" name="id_remetente" 	value="<?php print $row->id_remetente; ?>">
 	<div class="mb-3">
 		<label>Nome
 			<input type="text" name="nome_remetente" class="form-control" value="<?php print $row->nome_remetente; ?>">
 		</label>
 	</div>
 	<div class="mb-3">
 		<label>CPF/CNPJ
